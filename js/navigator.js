...
export class Navigator extends HTMLElement {
    ...
    checkForAppears() {
        ...
        if (appear.length) {
            appear[0].classList.replace('appear','appered');
            return true;
        }
        return false;
    }
    ...
    /**
     * resets for in-slide appearances on navigation
     */
    resetAppears() {
        const host = this.querySelector("div");
        const appeareds = host.querySelectorAll('.appered');
        appeareds.forEach((appear) =>
          {
